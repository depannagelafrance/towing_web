...
<div class="status--indication">
    <a class="<?php print $active_tab == 'for_invoice' || $active_tab == 'index' || $active_tab == '' ? 'active' : ''; ?>"
       href="/<?= $module ?>/overview/for_invoice">Ter facturatie</a>
    <a class="<?php print $active_tab == 'batch' ? 'active' : ''; ?>" href="/<?= $module ?>/overview/batch">Facturen</a>
</div>

<div class="layout-actions">
    <div class="btn--icon--highlighted bright">
        <a class="icon--add" href="/invoicing/invoice/create"
           onclick="return confirm('Bent u zeker dat u een nieuwe factuur wenst aan te maken?');">Nieuwe factuur
            aanmaken</a>
    </div>
</div>
...
<form method="post" action="/invoicing/export/expertm">
    ...
    <div class="box table_list table_list_large">
        ...
        $this->table->set_heading('Nummer', 'Type', 'Factuurdatum', 'Takelbon', 'Klant', 'Aanpassen', 'Download', 'Crediteer', 'Geëxporteerd?');
        ...
        if ($invoices && sizeof($invoices) > 0) {
            foreach ($invoices as $invoice) {
                ...
                $this->table->add_row(
                    ...
                    ($invoice->exported_to_expertm == 1 ? '<i class="fa fa-check" style="color: green;"></i>' : '<i class="fa fa-ban" style="color: red"></i>')
                );
            }
        }
